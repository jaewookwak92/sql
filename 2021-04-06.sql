 ...
 . 생성 -> OPEN -> FETCH -> CLOSE 순으로 처리해야함(단, FOR문은 예외)
  ...
  (1)생성
    ...
    DECLARE 
        ...
        V_BNAME PROD.BUYER.BUYER_NAME%TYPE;
        V_AMT NUMBER:=0;
    ...
    CURSOR CUR_BUY_INFO IS
        SELECT BUY_PROD,
                SUM(BUY_QTY)
            FROM BUYPROD
           WHERE BUY_DATE BETWEEN '20050301' AND '20050331' --날짜타입은 절대 LIKE쓰면안댐. BETWEEN 사용
           GROUP BY BUY_PROD;
